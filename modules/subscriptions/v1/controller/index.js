...
const { createSubscriptionSchema, updateSubscriptionSchema } = require("../validator/index");
...
module.exports.createSubscription = async (req, res) => {
    try {
        ...
        const response = await service.createSubscription(value);
        if (!response.status) {
            return res.status(400).json({ success: false, message: response.message });
        }
        
        return res.status(201).json({ success: true, message: "Subscription created successfully.", data: response.data });
    } catch (err) {
        console.error("Subscription Controller Error:", err);
...
module.exports.updateSubscription = async (req, res) => {
    try {
        ...
        const response = await service.updateSubscription(req.params.id, value);
        if (!response.status) {
            return res.status(400).json({ success: false, message: response.message });
        }
        
        return res.status(200).json({ success: true, message: "Subscription updated successfully.", data: response.data });
    } catch (err) {
        console.error("Subscription Controller Error:", err);
...
module.exports.deleteSubscription = async (req, res) => {
    try {
        const response = await service.deleteSubscription(req.params.id);
        if (!response.status) {
            return res.status(400).json({ success: false, message: response.message });
        }
