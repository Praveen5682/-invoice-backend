...
const { createReminderSchema, updateReminderSchema } = require("../validator/index");
...
module.exports.createReminder = async (req, res) => {
    try {
        ...
        return res.status(201).json({ success: true, message: "Reminder created successfully.", data: response.data });
    } catch (err) {
        console.error("Reminder Controller Error:", err);
...
module.exports.updateReminder = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Reminder updated successfully.", data: response.data });
    } catch (err) {
        console.error("Reminder Controller Error:", err);
...
module.exports.triggerReminder = async (req, res) => {
    try {
        ...
        console.error("Reminder Controller Error:", err);
        return res.status(500).json({ success: false, message: "Failed to trigger reminder." });
    }
};
